...
class EvoPointDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        if stage in ("fit", None):
            ...
            if self.hparams.use_target_domain and not self._has_pt_files(self.hparams.data_dir_af2):
                raise RuntimeError(
                    "Target-domain training is enabled but no AF2 .pt files were found in "
                    f"{self.hparams.data_dir_af2}. Disable data.use_target_domain or preprocess target samples."
                )
            if self.hparams.use_target_domain:
